...
if pagina == "🏠 Inicio":
    ...
    if estaciones is not None and mediciones is not None:
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.metric("Estaciones", f"{len(estaciones)}")
        with col2:
            st.metric("Mediciones PM2.5", f"{len(mediciones):,}")
        with col3:
...
# PÁGINA: ANÁLISIS TEMPORAL
    ...
    if mediciones is not None:
        ...
        datos_estacion['fecha_dia'] = datos_estacion['fecha'].dt.date
        datos_diarios = datos_estacion.groupby('fecha_dia')['pm25'].mean().reset_index()
        datos_diarios['fecha_dia'] = pd.to_datetime(datos_diarios['fecha_dia'])
        
        fig = px.line(
            datos_diarios,
            x='fecha_dia',
            y='pm25',
            title=f'Evolución de PM2.5 - {estacion_seleccionada}',
            labels={'fecha_dia': 'Fecha', 'pm25': 'PM2.5 (µg/m³)'}
        )
        ...
        with col1:
            st.metric("PM2.5 Promedio", f"{datos_diarios['pm25'].mean():.1f} µg/m³")
        with col2:
            st.metric("PM2.5 Máximo", f"{datos_diarios['pm25'].max():.1f} µg/m³")
        with col3:
            st.metric("PM2.5 Mínimo", f"{datos_diarios['pm25'].min():.1f} µg/m³")
...
# PÁGINA: ANÁLISIS POR MUNICIPIO
    ...
    if estaciones is not None and mediciones is not None and resumen_salud is not None:
